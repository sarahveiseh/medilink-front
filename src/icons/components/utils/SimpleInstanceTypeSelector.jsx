...
class SimpleInstanceTypeSelector extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    const { filteredListOfInstanceTypes, cpu, memory, count } = this.state;
        ...
        <GridItem xs={12}>
          <GridContainer>
            ...
            <GridItem md={2}>
              <CustomInput
                labelText="Count"
                id={`${props.formPrefix}.count`}
                name={`${props.formPrefix}.count`}
                formControlProps={{
                  fullWidth: true
                }}
                inputProps={{
                  value: props.values[`${props.formPrefix}`]["count"] || "",
...
export default withStyles(styles)(SimpleInstanceTypeSelector);
